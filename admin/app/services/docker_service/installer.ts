    ...
    if (service.service_name === SERVICE_NAMES.STREMIO) {
      ...
      if (gpuResult.type === 'nvidia') {
        ctx.broadcast(
          service.service_name,
          'gpu-config',
          `NVIDIA container runtime detected. Switching to NVENC-enabled Stremio image...`
        )
        finalImage = 'ghcr.io/tsaridas/stremio-docker:pr-142-gpu-nvidia-nvidia'
        const nvencImageExists = await ctx.checkImageExists(finalImage)
        if (!nvencImageExists) {
          ctx.broadcast(service.service_name, 'pulling', `Pulling NVENC image ${finalImage}...`)
          await ctx.pullImage(finalImage)
        }
        gpuHostConfig = {
          ...gpuHostConfig,

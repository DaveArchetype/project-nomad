...
export default class SystemController {
  ...
  async getAvailableVersions({ params, response }: HttpContext) {
    ...
    if (!service) {
      return response
        .status(404)
        .send({ error: `Service ${serviceName} not found or not installed` })
    }
  ...
  async deleteCustomApp({ request, response }: HttpContext) {
    ...
    await this.dockerService.removeCustomAppContainer(
      payload.service_name,
  ...
  async uninstallService({ request, response }: HttpContext) {
    ...
    if (service.is_dependency_service) {
      return response
        .status(403)
        .send({ error: 'Dependency services cannot be uninstalled directly.' })
    }
    if (!service.installed) {
      return response
        .status(409)
        .send({ error: `Service ${payload.service_name} is not installed` })
    }
  ...
  async setServiceCustomUrl({ request, response }: HttpContext) {
    ...
    if (!service) {
      return response
        .status(404)
        .send({ success: false, message: `Service ${payload.service_name} not found` })
    }
    // Hidden dependency services (e.g. Qdrant) aren't user-launchable, so they have no link to set.
    if (service.is_dependency_service) {
      return response
        .status(403)
        .send({ success: false, message: 'This service cannot be configured.' })
    }
    ...
    if (payload.custom_url && payload.custom_url.trim() && !normalized) {
      return response.status(422).send({
        success: false,
        message:
          'Custom URL must be a valid http(s) address (e.g. https://jellyfin.myhomelab.net).',
      })
    }
  ...
  async updateCustomApp_pullLatest({ request, response }: HttpContext) {
    ...
    if (!service.is_custom) {
      return response
        .status(403)
        .send({ success: false, message: 'Only custom apps can be updated this way.' })
    }
  ...
  async getServiceLogs({ params, request, response }: HttpContext) {
    ...
    if (!service) {
      return response
        .status(404)
        .send({ success: false, message: `Service ${params.name} not found` })
    }
  ...
  async updateCustomApp({ request, response }: HttpContext) {
    ...
    if (service.is_dependency_service) {
      return response
        .status(403)
        .send({ success: false, message: 'This service cannot be edited.' })
    }
    ...
    if (guard.blocked.length) {
      return response
        .status(422)
        .send({ success: false, message: guard.blocked.join(' '), blocked: guard.blocked })
    }
    if (!payload.force && guard.warnings.length) {
      return response
        .status(409)
        .send({ success: false, message: guard.warnings.join(' '), warnings: guard.warnings })
    }
    ...
    service.ui_location =
      prevScheme && uiLocation && /^\d+$/.test(uiLocation)
  ...
  private mergeCustomContainerConfig(
    ...
    containerConfig.HostConfig.RestartPolicy = containerConfig.HostConfig.RestartPolicy ?? {
      Name: 'unless-stopped',
    }
  ...
  private parseCustomContainerConfig(service: Service) {
    ...
    const ports = Object.entries(hostConfig.PortBindings ?? {}).map(
      ([key, val]: [string, any]) => ({
